...
import type { HttpMethod, HttpRequest } from 'simple-http-request-builder';
...
export default class ApiHttpClientAuthenticated {
  ...
  rawRequest(method: HttpMethod, path: string): HttpRequest<HttpPromise<Response>> {
    return this.configureRequest<HttpPromise<Response>>(this.httpClient.rawRequest(method, path));
  }

  restRequest<T>(method: HttpMethod, path: string): HttpRequest<HttpPromise<T>> {
    return this.configureRequest<HttpPromise<T>>(this.httpClient.restRequest<T>(method, path));
  }
}
