...
import type { ActionButtonProps, ActionContainerProps, ActionLinkProps, ThemeStyle } from '@lib/plume-admin-theme/action/ActionProps';
import ActionStyle from '@lib/plume-admin-theme/action/ActionStyle';
import { Button, CircularProgress, Icon } from '@mui/material';
import scss from './actions.module.scss';

function actionStyleToColor(actionStyle?: ActionStyle): 'inherit' | 'primary' | 'secondary' | 'error' {
  if (!actionStyle) {
    return 'inherit';
  }
...
export function ActionsContainer({ children, className, position = 'center', orientation = 'row' }: Readonly<ActionContainerProps>) {
  return <div className={classNames(scss.actions, className, scss[`actions--${position}`], scss[`actions--${orientation}`])}>{children}</div>;
}
...
export function ActionButton({
  ...
  return (
    <Button
      data-testid={dataTestId}
      onClick={onClick}
      className={classNames(scss.action, className, isLoading ? scss.actionLoading : undefined, typeClassName(style, variant))}
