...
import type { ProgressData } from './useExportProgress';
...
export default function TransferProgressDisplay({ transferProgress, isLikes }: Readonly<TransferProgressDisplayProps>) {
  ...
  return (
    <div className={scss.transferProgress}>
      {transferProgress.status === 'transferring' && (
        <>
          <span>{`${transferProgress.songName}`}</span>
          <LinearProgress
            variant="determinate"
            value={transferProgress.percentage}
            sx={{ width: '100%', marginTop: '10px' }}
          />
          <span>{`${transferProgress.currentSong} / ${transferProgress.totalSongs}`}</span>
        </>
      )}
      {transferProgress.status === 'completed' && (
        <>
          ...
          {(transferProgress.spotifyPlaylistId || isLikes) && (
            <a
              href={
                isLikes ? `https://open.spotify.com/collection/tracks` : `https://open.spotify.com/playlist/${transferProgress.spotifyPlaylistId}`
              }
              target="_blank"
      ...
      {transferProgress.status === 'error' && <p style={{ color: 'red' }}>Transfer failed.</p>}
